	...
	tests := []test{
		{
			...
			args: args{
				...
				envs: map[string]string{
					"TUNNEL_SCANNERS":   "vuln",
					"TUNNEL_SKIP_FILES": "path/to/dummy,testdata/fixtures/repo/gomod/submod2/go.mod",
				},
				configFile: `---
scan:
  scanners:
...
`,
		...
		},
		{
			...
			args: args{
				...
				envs: map[string]string{
					"TUNNEL_SCANNERS":      "misconfig",
					"TUNNEL_FILE_PATTERNS": "dockerfile:Customfile",
					"TUNNEL_NAMESPACES":    "testing",
				},
				configFile: `---
scan:
  scanners:
...
`,
		...
		},
		{
			...
			args: args{
				...
				envs: map[string]string{
					"TUNNEL_SCANNERS": "vulnerability",
					"TUNNEL_SEVERITY": "CRITICAL,INVALID",
				},
				configFile: `---
scan:
  scanners:
...
`,
	...
	for _, tt := range tests {
		...
		t.Run(tt.name+" with environmental variables", func(t *testing.T) {
			...
			t.Setenv("TUNNEL_OUTPUT", outputFile)
			t.Setenv("TUNNEL_FORMAT", "json")
			t.Setenv("TUNNEL_CACHE_DIR", cacheDir)
			t.Setenv("TUNNEL_SKIP_DB_UPDATE", "true")
			t.Setenv("TUNNEL_SKIP_POLICY_UPDATE", "true")
...
`, outputFile, cacheDir)
			...
			configPath := filepath.Join(t.TempDir(), "tunnel.yaml")
