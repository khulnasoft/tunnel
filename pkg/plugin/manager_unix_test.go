...
import (
	...
	"github.com/khulnasoft/tunnel/internal/gittest"
...
`
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// For plugin index
			pluginDir := filepath.Join(dst, ".tunnel", "plugins")
			err := os.MkdirAll(pluginDir, 0755)
			require.NoError(t, err)
			_, err = fsutils.CopyFile("testdata/.tunnel/plugins/index.yaml", filepath.Join(pluginDir, "index.yaml"))
			require.NoError(t, err)
