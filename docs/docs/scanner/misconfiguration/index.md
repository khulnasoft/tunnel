...
```bash
tunnel config --misconfig-scanners=terraform,dockerfile .
```
...
```bash
tunnel config --config-check custom-policy/policy --config-check combine/policy --config-check policy.rego --namespaces user myapp
```
...
$ tunnel config --misconfig-scanners=json,yaml --config-check ./serverless.rego --check-namespaces user ./iac
...
tunnel config --config-check ./my-check --data ./data --namespaces user ./configs
```
...
#tunnel:ignore:aws-ec2-no-public-ingress-sgr[from_port=3306]
...
```tf
#tunnel:ignore:aws-ec2-no-public-ingress-sgr[from_port=3306]
#tunnel:ignore:aws-ec2-no-public-ingress-sgr[from_port=5432]
```
...
#tunnel:ignore:aws-ec2-no-public-ingress-sgr[from_port=1000,type=egress]
resource "aws_security_group_rule" "example" {
...
```tf
#tunnel:ignore:*[logging_config.prefix=myprefix]
...
```tf
#tunnel:ignore:aws-s3-enable-logging
module "s3_bucket" {
