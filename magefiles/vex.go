...
func run() error {
	...
	cloneDir := flag.String("dir", "tunnel", "Directory to clone the repository")
	output := flag.String("output", ".vex/tunnel.openvex.json", "Output file")
	flag.Parse()

	ctx := context.Background()

	// Clone or pull the Tunnel repository
	...
	// Maps to store "not_affected" statements across Tunnel versions
	notAffectedVulns := make(map[UniqueKey][]vex.Statement)

	// Indicate one or more Tunnel versions are affected by the vulnerability.
...
// cloneOrPullRepo clones the Tunnel repository or pulls updates if it already exists
...
func processTag(ctx context.Context, tag string) (map[UniqueKey]vex.Statement, map[UniqueKey]struct{}, error) {
	...
	product := *productID // Clone Tunnel PURL
	product.Version = tag
	...
	for key, stmts := range notAffected {
		for _, stmt := range stmts {
			if _, ok := affected[key]; !ok {
				...
				// => pkg:golang/github.com/khulnasoft/tunnel
			...
			// => pkg:golang/github.com/khulnasoft/tunnel@0.52.0
