...
func (s *SPDX) unmarshal(spdxDocument *spdx.Document) error {
	s.tunnelSBOM = s.isTunnelSBOM(spdxDocument)

	// Parse files and find file paths for packages
	s.parseFiles(spdxDocument)

	// Convert all SPDX packages into Tunnel components
...
func (s *SPDX) parsePackage(spdxPkg spdx.Package) (*core.Component, error) {
	...
	// Tunnel stores properties in Annotations
	...
	// Older Tunnel versions put the file path in "sourceInfo" and the package type in "name".
	if s.tunnelSBOM && component.Type == core.TypeApplication && spdxPkg.PackageSourceInfo != "" {
...
func (s *SPDX) isTunnelSBOM(spdxDocument *spdx.Document) bool {
	...
	for _, c := range spdxDocument.CreationInfo.Creators {
		if c.CreatorType == "Tool" && strings.HasPrefix(c.Creator, "tunnel") {
			return true
		}
	}
	return false
