...
const indexURL = "https://khulnasoft.github.io/tunnel-plugin-index/v1/index.yaml"
...
func (m *Manager) Search(ctx context.Context, keyword string) error {
	...
	if errors.Is(err, os.ErrNotExist) {
		m.logger.ErrorContext(ctx, "The plugin index is not found. Please run 'tunnel plugin update' to download the index.")
...
func (m *Manager) tryIndex(ctx context.Context, name string, opts Options) string {
	...
	index, err := m.loadIndex()
	if errors.Is(err, os.ErrNotExist) {
		m.logger.WarnContext(ctx, "The plugin index is not found. Please run 'tunnel plugin update' to download the index.")
		return name
	} else if err != nil {
