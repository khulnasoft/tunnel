...
.PHONY: protoc
protoc:
	docker build -t tunnel-protoc - < Dockerfile.protoc
	docker run --rm -it -v ${PWD}:/app -w /app tunnel-protoc make _$@
...
.PHONY: label
label: $(GOBIN)/labeler
	labeler apply misc/triage/labels.yaml -r khulnasoft/tunnel -l 5
