...
func (c *BOM) UnmarshalJSON(b []byte) error {
	...
	if !core.IsTunnelSBOM(bom) {
		log.Logger.Warnf("Third-party SBOM may lead to inaccurate vulnerability detection")
		log.Logger.Warnf("Recommend using Tunnel to generate SBOMs")
	}
	...
	if bom.Metadata != nil {
		...
		if bom.Metadata.Component != nil {
			metadata.Component = toTunnelCdxComponent(lo.FromPtr(bom.Metadata.Component))
		}
	}

	var components []ftypes.Component
	for _, component := range lo.FromPtr(bom.Components) {
		components = append(components, toTunnelCdxComponent(component))
	}
...
func (c *BOM) parseSBOM(bom *cdx.BOM) error {
	...
	for bomRef := range c.dependencies {
		...
		case cdx.ComponentTypeApplication: // It would be a lock file in a CycloneDX report generated by Tunnel
...
func toPackage(component cdx.Component) (*purl.PackageURL, *ftypes.Package, error) {
	...
	pkg := p.Package()
	// Tunnel's marshall loses case-sensitivity in PURL used in SBOM for packages (Go, Npm, PyPI),
...
func toTunnelCdxComponent(component cdx.Component) ftypes.Component {
