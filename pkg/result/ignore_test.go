...
func TestParseIgnoreFile(t *testing.T) {
	t.Run("happy path valid config file", func(t *testing.T) {
		got, err := ParseIgnoreFile(context.TODO(), "testdata/.tunnelignore")
		require.NoError(t, err)
		assert.Equal(t, "testdata/.tunnelignore", got.FilePath)
	...
	t.Run("happy path valid YAML config file", func(t *testing.T) {
		got, err := ParseIgnoreFile(context.TODO(), "testdata/.tunnelignore.yaml")
		require.NoError(t, err)
		assert.Equal(t, "testdata/.tunnelignore.yaml", got.FilePath)
	...
	t.Run("invalid file passed", func(t *testing.T) {
		...
		require.NoError(t, err)
		defer os.Remove(f.Name())
		_, _ = f.WriteString("this file is not a valid tunnelignore file")
