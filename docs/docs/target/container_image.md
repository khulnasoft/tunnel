...
Tunnel supports two targets for container images.
...
Tunnel scans the files inside container images for
...
```shell
$ tunnel image --scanners vuln [YOUR_IMAGE_NAME]
```
...
```
$ tunnel image --scanners misconfig [YOUR_IMAGE_NAME]
```
...
```shell
$ tunnel image --scanners license [YOUR_IMAGE_NAME]
```
...
Tunnel scans the configuration of container images for
...
```
$ tunnel image --image-config-scanners misconfig [YOUR_IMAGE_NAME]
```
...
|----------|------------|--------|
| [AVD-DS-0007](https://avd.khulnasoft.com/misconfig/dockerfile/general/avd-ds-0007/) | This check detects multiple `ENTRYPOINT` instructions in a stage, but since image history analysis does not identify stages, this check is not relevant for this scan type. | [#8364](https://github.com/khulnasoft/tunnel/issues/8364) |
| [AVD-DS-0016](https://avd.khulnasoft.com/misconfig/dockerfile/general/avd-ds-0016/) | This check detects multiple `CMD` instructions in a stage, but since image history analysis does not identify stages, this check is not relevant for this scan type. | [#7368](https://github.com/khulnasoft/tunnel/issues/7368) |
...
```shell
$ tunnel image --image-config-scanners secret [YOUR_IMAGE_NAME]
```
...
Tunnel will look for the specified image in a series of locations. By default, it
...
```bash
tunnel image --image-src podman,containerd alpine:3.7.3
```

Will first search in Podman. If the image is found there, it will be scanned
and the results returned. If the image is not found in Podman, then Tunnel will
...
You can configure credentials with `tunnel registry login`.
...
$ tunnel image --input /path/to/alpine
```
...
$ tunnel image --input /path/to/alpine@sha256:82389ea44e50c696aba18393b168a833929506f5b29b9d75eb817acceb6d54ba
```
...
```bash
$ tunnel image --sbom-sources oci ghcr.io/knqyf263/oci-referrers
...
```
tunnel image --compliance docker-cis-1.6.0 [YOUR_IMAGE_NAME]
```
...
```
$ tunnel image --platform=os/architecture [YOUR_IMAGE_NAME]
```

For example:

```
$ tunnel image --platform=linux/arm alpine:3.16.1
```
...
```shell
$ tunnel image --docker-host tcp://127.0.0.1:2375 YOUR_IMAGE
```
...
$ tunnel image --max-image-size=10GB myapp:latest
```
