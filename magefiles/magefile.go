...
func buildLdflags() (string, error) {
	...
	return fmt.Sprintf("-s -w -X=github.com/khulnasoft/tunnel/pkg/version/app.ver=%s", ver), nil
}
...
func Protoc() error {
	...
	if _, ok := os.LookupEnv("TUNNEL_PROTOC_CONTAINER"); ok {
	...
	if err := sh.RunV("bash", "-c", "docker build -t tunnel-protoc - < Dockerfile.protoc"); err != nil {
		return err
	}
	return sh.Run("docker", "run", "--rm", "-it", "--platform", "linux/x86_64", "-v", "${PWD}:/app", "-w", "/app", "tunnel-protoc", "mage", "protoc")
}
...
// Build builds Tunnel
func Build() error {
	if updated, err := target.Dir("tunnel", "pkg", "cmd"); err != nil {
		return err
	} else if !updated {
		return nil
	}
	...
	return sh.RunWith(ENV, "go", "build", "-ldflags", ldflags, filepath.Join(wd, "cmd", "tunnel"))
}

// Install installs Tunnel
func Install() error {
	ldflags, err := buildLdflags()
	if err != nil {
		return err
	...
	return sh.RunWith(ENV, "go", "install", "-ldflags", ldflags, filepath.Join(wd, "cmd", "tunnel"))
}
...
func (Docs) Css() error {
	const (
		homepageSass = "docs/assets/css/tunnel_v1_homepage.scss"
	)
