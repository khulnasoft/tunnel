...
Tunnel can generate the following SBOM formats.
...
```
$ tunnel image --format spdx-json --output result.json alpine:3.15
```

```
$ tunnel fs --format cyclonedx --output result.json /app/myproject
```
...
Tunnel can generate SBOM in the [CycloneDX][cyclonedx] format.
...
```
$ tunnel image --format cyclonedx --output result.json alpine:3.15
...
```
$ tunnel image --scanners vuln --format cyclonedx --output result.json alpine:3.15
```

#### SPDX

Tunnel can generate SBOM in the [SPDX][spdx] format.

You can use the regular subcommands (like `image`, `fs` and `rootfs`) and specify `spdx` with the `--format` option.

```
$ tunnel image --format spdx --output result.spdx alpine:3.15
```
...
```
$ tunnel image --format spdx-json --output result.spdx.json alpine:3.15
```
...
$ cat result.spdx.json | jq .
{
	...
	"dataLicense": "CC0-1.0",
	"documentNamespace": "http://khulnasoft.github.io/tunnel/container_image/alpine:3.15-d9549e3a-a4c5-4ee3-8bde-8c78d451fbe7",
	"name": "alpine:3.15",
	"packages": [
		{
			"SPDXID": "SPDXRef-12bc938ac028a5e1",
...
Tunnel can take SBOM documents as input for scanning, e.g `tunnel sbom ./sbom.spdx`.
See [here](../target/sbom.md) for more details.

### SBOM Detection inside Targets

Tunnel searches for SBOM files in container images with the following extensions:
...
In addition, Tunnel automatically detects SBOM files in [Bitnami images](https://github.com/bitnami/containers), [see here](../coverage/others/bitnami.md) for more details.
...
When scanning container images, Tunnel can discover SBOM for those images. [See here](../target/container_image.md) for more details.
