...
The check id field (`controls[].checks[].id`) is referring to existing check by it's "AVD ID". This AVD ID is easily located in the check's source code metadata header, or by browsing [Aqua vulnerability DB](https://avd.khulnasoft.com/), specifically in the [Misconfigurations](https://avd.khulnasoft.com/misconfig/) and [Vulnerabilities](https://avd.khulnasoft.com/nvd) sections.
