...
import (
	...
	"github.com/khulnasoft/tunnel/pkg/commands/clean"
	...
	"github.com/khulnasoft/tunnel/pkg/version"
	"github.com/khulnasoft/tunnel/pkg/version/app"
	vexrepo "github.com/khulnasoft/tunnel/pkg/vex/repo"
...
func NewRepositoryCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ tunnel repo https://github.com/knqyf263/tunnel-ci-test
...
func NewClientCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			log.Warn("'client' subcommand is deprecated now. See https://github.com/khulnasoft/tunnel/discussions/2119")
...
func NewPluginCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ tunnel plugin install github.com/aquasecurity/tunnel-plugin-referrer`,
		...
		&cobra.Command{
			...
			SilenceErrors:         true,
			SilenceUsage:          true,
			Short:                 "List Tunnel plugins available on the plugin index and search among them",
