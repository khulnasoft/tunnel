...
Tunnel scans any container image for license files and offers an opinionated view on the risk associated with the license.
...
```shell
$ tunnel image --scanners license --ignored-licenses MPL-2.0,MIT --severity HIGH grafana/grafana:latest
...
```shell
$ tunnel fs --scanners license --license-full <path/to/spdx/list/data>
...
```shell
$ tunnel fs --scanners license --license-full --license-confidence-level 0.8 <path/to/spdx/list/data>
...
```shell
$ tunnel image --generate-default-config
$ vim tunnel.yaml
license:
  forbidden:
  - AGPL-3.0
