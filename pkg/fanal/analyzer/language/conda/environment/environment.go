...
import (
	...
	"github.com/khulnasoft/tunnel/pkg/dependency/parser/conda/environment"
	"github.com/khulnasoft/tunnel/pkg/dependency/parser/conda/meta"
	...
	for i, pkg := range pkgs.Packages {
		...
		if pkg.Version != "" {
			...
			if err != nil {
				...
				once.Do(func() {
					// e.g. https://khulnasoft.github.io/tunnel/latest/docs/coverage/os/conda/#license_1
...
func findLicenseFromEnvDir(pkg types.Package, prefix string) ([]string, error) {
	...
	for _, entry := range entries {
		...
		if matched {
			...
			// cf. https://github.com/khulnasoft/tunnel/blob/c3192f061d7e84eaf38df8df7c879dc00b4ca137/pkg/dependency/parser/conda/meta/parse.go#L39-L45
