...
import (
	...
	"github.com/khulnasoft/tunnel/pkg/licensing/expression"
...
const (
	...
	DocumentNamespace      = "http://tunnel.khulnasoft.com"
...
type Marshaler struct {
	...
	hasher     Hash
	appVersion string // Tunnel version. It needed for `creator` field
	logger     *log.Logger
}
...
func (m *Marshaler) packageDownloadLocation(root *core.Component) string {
	...
	if root.Type == core.TypeRepository {
		// Tunnel currently only supports git repositories. Format examples:
...
func (m *Marshaler) normalizeLicenses(licenses []string) (string, []*spdx.OtherLicense) {
	...
	license := strings.Join(lo.Map(licenses, func(license string, index int) string {
		...
		// as an example, parse a license with 2 consecutive tokens (see https://github.com/khulnasoft/tunnel/issues/8465)
