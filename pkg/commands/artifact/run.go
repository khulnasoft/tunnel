...
import (
	...
	"github.com/khulnasoft/tunnel/pkg/javadb"
	...
	pkgReport "github.com/khulnasoft/tunnel/pkg/report"
...
func Run(ctx context.Context, opts flag.Options, targetKind TargetKind) (err error) {
	...
	defer func() {
		if errors.Is(err, context.DeadlineExceeded) {
			// e.g. https://tunnel.khulnasoft.com/latest/docs/configuration/
	...
	if opts.ServerAddr != "" && opts.Scanners.AnyEnabled(types.MisconfigScanner, types.SecretScanner) {
		log.WarnContext(ctx,
			fmt.Sprintf(
				"Tunnel runs in client/server mode, but misconfiguration and license scanning will be done on the client side, see %s",
	...
	if opts.GenerateDefaultConfig {
		log.Info("Writing the default config to tunnel-default.yaml...")
		...
		return v.SafeWriteConfigAs("tunnel-default.yaml")
	}
...
func disabledAnalyzers(opts flag.Options) []analyzer.Type {
	...
	if os.Getenv("TUNNEL_EXPERIMENTAL_RPM_ARCHIVE") == "" {
...
func (r *runner) initScannerConfig(ctx context.Context, opts flag.Options) (ScannerConfig, types.ScanOptions, error) {
	...
	if opts.Scanners.Enabled(types.SecretScanner) {
		...
		// e.g. https://tunnel.khulnasoft.com/latest/docs/scanner/secret/#recommendation
