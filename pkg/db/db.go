...
var (
	...
	DefaultGHCRRepository = fmt.Sprintf("%s:%d", "ghcr.io/khulnasoft-lab/tunnel-db", db.SchemaVersion)
...
func (c *Client) NeedsUpdate(ctx context.Context, cliVersion string, skip bool) (bool, error) {
	...
	if db.SchemaVersion < meta.Version {
		log.ErrorContext(ctx, "Tunnel version is old. Update to the latest version.", log.String("version", cliVersion))
...
func (c *Client) validate(meta metadata.Metadata) error {
	if db.SchemaVersion != meta.Version {
		log.Error("The local DB has an old schema version which is not supported by the current version of Tunnel CLI. DB needs to be updated.")
